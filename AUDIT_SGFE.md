...
- [ ] 🔗 Vrais **tests e2e Playwright**. *(M)* — **🟡 Partiel, sensiblement rapproché du fait.** PR #143 (31/08) ajoute un spec saisie-index (gaté `E2E_LIVE_BACKEND=1`). Backend PR #189 (04/09) ajoute `WHATSAPP_DISABLE_SEND_FOR_TESTS` (simule l'envoi WhatsApp sans contacter le vrai `whatsapp-service`) ; le même jour le frontend lève le `test.skip()` du spec paiement et ajoute 3 specs (`espace-abonne`, `dashboard-consultation`, `abonnes-gestion`), les 4 gatées `E2E_LIVE_BACKEND`. **Corrigé le 05/09/2026** : le job `e2e` frontend (`SGFE-frontend#169`) démarre désormais une vraie stack `SGFE-backend` éphémère et pose `E2E_LIVE_BACKEND=1` — mais à cette date, un seul des 4 specs (`dashboard-consultation`) avait de quoi passer réellement : `terrain-saisie-index` (aucune campagne `EN_COURS` semée, aucun relevé), `abonnes-gestion` (aucun abonné semé) et `paiement-encaissement` (bug de date rendant `/impayes` vide avant le 16 de chaque mois) étaient bloqués par de vrais manques de données de démo, identifiés précisément plutôt que masqués. **PR #217 (même jour)** corrige les 3 causes exactes côté `scripts/seed/` (nouveau `abonne.py`, campagne `EN_COURS` avec relevés, date d'impayé relative à `date.today()`) — vérifié par relecture directe de l'état en base après seed contre un Postgres jetable, comparé ligne à ligne à ce que chaque spec attend. **Reste à confirmer** : l'exécution Playwright réelle des 3 specs au prochain run CI live après fusion (même discipline que SLSA Build L3, PR #209 — incertitude assumée, à lever par l'observation).
