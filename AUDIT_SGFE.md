...
> ⚠️ **6e revue de fraîcheur — 5 septembre 2026 : plan de remédiation des deux
> audits (5e revue) exécuté — 6 PR fusionnées, la quasi-totalité des écarts
> **Critique**/Élevé/Moyen des deux rapports sont désormais fermés.**
>
> - **§8·A repasse de 🟡 partiel à ✅ fait** — PR #198 : les 5 valeurs de repli
>   insécures (`PII_ENCRYPTION_KEY`, `DJANGO_SECRET_KEY`, `INTERNAL_GRPC_KEY`,
>   `POSTGRES_PASSWORD`, `WHATSAPP_INTERNAL_API_KEY`) retirées de
>   `docker-compose.yml`, remplacées par `${VAR:?message}` — le fail-fast
>   Python se déclenche désormais réellement, plus de secret exploitable en
>   clair dans le dépôt. Migration de rotation de la clé PII compromise
...
>   --audit-level=high`, `eslint-plugin-security`, `permissions: contents:
>   read`. *(item CICD #1, #7, #9, #10, #11, #13.)*
> - **Défense en profondeur gRPC** (écart #3 du rapport OWASP/SOC2, narré en
>   §J ci-dessus, sans item dédié) — PR #199 ajoute une revalidation du rôle
>   appelant (via l'identité propagée par PR #193) sur les 5 RPC les plus
>   sensibles (`EnregistrerPaiement`, `AnnulerPaiement`,
>   `EnregistrerPaiementAbonne`, `DeactivateUser`, `UpdateTarif`) —
>   volontairement **non bloquante** (log d'avertissement seul) tant que la
>   propagation d'identité ne couvre pas tous les appels légitimes ; la
>   gateway reste l'unique décideur RBAC.
> - **mTLS — repli en clair silencieux corrigé** (écart #4) — même PR #199 :
>   nouveau `GRPC_TLS_REQUIRED`, avertissement journalisé à chaque repli en
>   clair (jusqu'ici muet), refus explicite au démarrage possible en
>   production. La ✅ déjà posée en §8·B (PR #168) n'est pas retouchée, ce
>   correctif referme un angle mort découvert après coup par le rapport.
> - **Chiffrement PII étendu, partiellement** (écart #5) — PR #201/#202 :
>   `services/notification/notifications/models.py` (`Envoi.telephone`,
>   `DiffusionEnvoi.telephone`) chiffré au repos (Fernet, même patron que
>   §8·G), migration de rechiffrement des données existantes incluse.
>   **`services/auth/comptes/models.py` (`email`, `phone_number`) reste
>   délibérément non chiffré** : ce sont des colonnes `unique=True` en
>   recherche exacte (connexion, réinitialisation) et un chiffrement Fernet
>   non déterministe casserait à la fois l'unicité et les lookups — écart
>   documenté, pas corrigé, jusqu'à un futur chiffrement déterministe ou
>   HMAC-indexé.
> - **Throttle OTP/réinitialisation ajouté** (écart #7) — PR #200 : nouveau
>   `services/auth/comptes/throttle.py` (verrou Redis `SET NX EX`, 1 requête/
>   60 s), câblé sur `request_otp_by_phone` et `request_password_reset` — la
>   ✅ déjà posée en ligne « Rate limiting » ci-dessous se complète enfin du
>   throttle par identifiant métier qui manquait, en plus du `limit_req` nginx
>   par IP.
> - **Validation GraphQL renforcée** (écart #10) — même PR #200 : nouveau
>   `gateway/schema/validators.py` (téléphone E.164, index, dates), appliqué
>   aux champs `@strawberry.input` critiques.
> - **Provenance SLSA vérifiée** (écart #8) — même PR #200 : `cosign
>   verify-attestation --type slsaprovenance` ajouté à `cd-prod.yml`, en plus
>   de la vérification de signature déjà en place.
>
> **Restent ouverts, sans changement** : SHA-pinning/`allowed_actions`
> (réglages GitHub, pas du code — item CICD #4, délibérément non modifié vu
> l'impact opérationnel), Dependabot security updates (réglage, item CICD
> #5), `CODEOWNERS`/`SECURITY.md` (item CICD #6), migration SLSA Build L3
> (item CICD #12, effort L), RGPD utilisateurs internes (écart #6, bloqué sur
> une décision de durée de rétention légale), extension de la piste d'audit
> §J à campagne/abonné/config (écart #1), et les trois écarts Faible (cookie
> `Secure` littéral, `ssl_ciphers` nginx, conversion `int()` hors
> `try/except` dans `whatsapp_client.py:198`).
>
> **Décompte mis à jour (96 items, §8, recompté par script)** : **76 faits (79
> %)** · **5 partiels** · **7 incertains/non revérifiés** · **8 non faits**.
> Seuls §8·A et §8·K changent de colonne (🟡→✅) ; les autres correctifs
> ci-dessus n'ont pas d'item dédié dans la checklist et sont donc déjà
> comptés ailleurs (défense en profondeur, PII notification, throttle,
> validation, provenance — narratifs uniquement, voir §J et les deux rapports
> de conformité pour le détail vérifiable `fichier:ligne`).

> ⚠️ **7e revue de fraîcheur — 5 septembre 2026 : piste d'audit étendue aux 6
> services prévus, immuabilité corrigée en profondeur (bug pire que documenté
> trouvé et fermé), RGPD étendu aux utilisateurs internes, SLSA Build L3
> vérifié en conditions réelles, un vrai bug de porte de fusion trouvé et
> corrigé.** 11 PR fusionnées (#205 à #215).
>
> - **§J — Journal d'audit métier immuable : 🟡 partiel → ✅ fait.** Les 4
>   services que la conception §10.7 listait comme restants sont livrés :
>   config (PR #205), campagne (PR #208), auth (PR #210), abonné (PR #212) —
>   même patron que paiement/facturation (PR #193), écriture `AuditLog` dans
>   la même transaction que la mutation métier. Reporting (read-only) et
>   Notification (hors périmètre de la conception) n'en reçoivent
>   délibérément pas.
> - **Nouvel item — Immuabilité réelle du journal d'audit : ✅ fait, PR #215.**
>   La limite documentée depuis le 04/09 (« le rôle applicatif est propriétaire
>   de la table ») était **pire que ça** : le rôle Postgres est en réalité
>   **superutilisateur**, ce qui contourne toute ACL indépendamment de la
>   propriété — trouvé et vérifié empiriquement sur un Postgres jetable.
>   Corrigé pour paiement/facturation par un second rôle `NOLOGIN` non
>   superutilisateur (`SET ROLE` par connexion, aucun changement de
>   `docker-compose.yml`), vérifié par une vraie suite d'intégration Postgres.
>   Campagne/config/auth/abonné gardent l'ancien REVOKE symbolique — mécanisme
>   correctif générique déjà prêt (`libs/sgfe_common/sgfe_common/db_hardening.py`)
>   pour qu'un futur correctif l'applique aux 4 sans le redécouvrir.
> - **RGPD étendu aux utilisateurs internes — PR #213.** Export/anonymisation
>   sur le même principe que l'Abonné (PR #179), plus un job de purge
>   automatique quotidien après **3 ans de désactivation** — durée validée
>   explicitement par l'utilisateur du projet, pas devinée.
> - **SLSA Build L3 (item CICD #12) — ✅ fait ET vérifié en conditions réelles,
>   PR #209.** Génération de la provenance isolée du job de build via
>   `slsa-framework/slsa-github-generator`. L'agent avait honnêtement signalé
>   une incertitude réelle (identité OIDC non vérifiable statiquement) —
>   **au premier `push` réel sur `develop` après fusion, les 11 jobs
>   `publish-*` et leur job de provenance imbriqué (`detect-env → generator →
>   final`) ont tous réussi**, sans exception. Incertitude levée par
>   l'observation, pas par confiance a priori.
> - **Test de charge réaliste — PR #211.** `loadtest/parcours-metier.js`
>   (nouveau, backend) remplace le script jouet de 2-3 requêtes par un
>   parcours métier complet (login, listes paginées, dashboard), profil de
>   charge dimensionné sur la vraie limite nginx (`30r/s burst=60`).
> - **Bug réel trouvé et corrigé dans le job `ci-status` lui-même (les deux
>   dépôts).** `contains(needs.*.result, 'failure')` ne détecte pas un job
>   **annulé** (ex. un timeout) — constaté en direct côté frontend (PR
>   SGFE-frontend#167, Playwright annulé à 20m16s par un mirroir apt
>   anormalement lent) : `ci-status` avait quand même conclu au succès.
>   `cancelled` bloque désormais comme `failure` sur les deux dépôts (backend
>   PR #214). Ce job protège la porte de fusion de `develop` depuis PR #197 —
>   ce défaut existait donc, silencieux, depuis cette date.
> - **e2e paiement WhatsApp** : le garde-fou `WHATSAPP_DISABLE_SEND_FOR_TESTS`
>   documenté comme non branché s'est révélé déjà entièrement câblé (PR #189,
>   04/09) — écart de documentation dans cette checklist, corrigé (PR #206),
>   aucun code applicatif changé. Le scénario e2e « backend live » en CI
>   frontend est ébauché (PR SGFE-frontend#169) : sur 4 specs gatées, une
>   seule (dashboard) peut réellement passer aujourd'hui — les 3 autres restent
>   bloquées par de vrais manques de données de démo côté backend (campagnes
>   toujours `CLOTUREE`, aucun abonné semé, un bug de date dans le seed des
>   impayés), documentés précisément, pas de faux signal vert.
> - **4 correctifs mineurs + hygiène (PR #207)** : cookie `refresh_token`
>   `Secure=True` en dur, `ssl_ciphers` nginx explicite, correction
>   `whatsapp_client.py:198` (conversion hors `try/except`), doc `CLAUDE.md`
>   (15 min réel, pas 24h) — écarts Faible du rapport OWASP/SOC2, tous fermés.
>   `SECURITY.md`/`CODEOWNERS` ajoutés aux deux dépôts (item CICD #6).
>
> **Restent ouverts, sans changement** : observabilité (§I, toujours à
> zéro — seul vrai blocage SOC 2/« Go production »), réglages GitHub
> (SHA-pinning obligatoire, allowlist des actions, Dependabot, protection de
> branche `main`/`develop`) — **bloqués par le classificateur de permissions de
> l'environnement d'exécution de cette revue, pas par choix** : toute
> tentative de modification via l'API GitHub a été refusée, quelle que soit
> l'autorisation donnée en conversation ; commandes exactes remises à
> l'utilisateur pour exécution manuelle.
>
> **Décompte mis à jour (97 items, §8, recompté par script — le total passe de
> 96 à 97 : l'immuabilité réelle du journal d'audit compte comme un item
> propre, même principe que les incidents du 3 septembre)** : **80 faits (82
> %)** · **3 partiels** · **6 incertains/non revérifiés** · **8 non faits**.
>
> **Mise à jour du 5 septembre 2026 (i18n backend) : §Q passe de non-fait à
> fait** — 113 littéraux utilisateur (erreurs de validation métier, messages
> d'exception remontés jusqu'à une réponse gRPC) externalisés via
> `gettext_lazy` sur les 8 services, mécanisme Django standard câblé
> (`LANGUAGE_CODE`, `USE_I18N`, `LOCALE_PATHS`, `locale/` par service) —
> **externalisation seulement, aucune traduction fournie**. 1386/1386 tests
> verts, aucune modification de test. Décompte recompté par script : **81
> faits (84 %)** · **3 partiels** · **6 incertains/non revérifiés** · **7
> non faits**.
>
> **Mise à jour du 5 septembre 2026 (journalisation de sécurité + collecte de
> preuves) : §J avance encore.** Les événements de sécurité de la gateway
> (refus de rôle, jeton invalide — jusqu'ici seulement dans un logger local)
> sont désormais aussi centralisés dans l'`AuditLog` du service Auth (nouveau
> RPC `EnregistrerEvenementSecurite`, appel best-effort/non bloquant — le
> logger local reste le filet de secours, jamais remplacé). Nouveau mécanisme
> générique de chaînage de hash (`libs/sgfe_common/sgfe_common/log_integrity.py`)
> rend les logs locaux détectables-si-modifiés a posteriori — câblé sur
> Auth+Gateway (2/9 composants, extension triviale documentée) ; honnêteté
> gardée sur ses limites : détection après coup, pas un WORM, pas de signature
> externe, pas de persistance cross-redémarrage. Nouveau `docs/PREUVES_SOC2.md`
> déclare formellement, service par service, quelles preuves sont collectées
> et depuis quand. 552/552 tests concernés verts (auth 207, gateway 345),
> aucune régression. **Décompte final recompté par script, les deux PR
...
> **Complément du 05/09/2026, sur demande explicite** (« il y a des tâches
> qui sont encore à partiel, il faut les régler ») : deux passages
> supplémentaires sur le pentest-lite (mTLS inter-services testé en direct
> depuis l'intérieur du réseau Docker, IDOR sur les 3 routes tokenisées de
> l'espace abonné, sondage d'injection ciblé — `SGFE-Backend#222`, item
> reste `partiel`, les gaps structurels restants sont honnêtes) et **les
> tests e2e Playwright confirmés réellement en direct** (`SGFE-frontend#175`)
> — 4 itérations de CI, 4 causes réelles trouvées et corrigées une par une,
> dont **un vrai bug de production découvert par ce travail** : l'écran
> `/impayes` était cassé en permanence pour tout utilisateur COMPTABLE
> (appel à une query réservée ADMIN dans le chargement de la page), sans
> rapport avec le seed ni avec l'e2e lui-même — un bug qui existait déjà en
> production, révélé uniquement parce que le parcours COMPTABLE a enfin été
> testé en conditions réelles plutôt que supposé fonctionnel. Item e2e
> Playwright passé de partiel à **fait**. Des trois items encore partiels,
> deux restent partiels par nature et non par manque de travail : la
> collecte de preuves SOC 2 est *(continu)* par construction (une période
> d'observation ne se clôt pas sur commande), et le pentest-lite ne
> remplacera jamais un vrai test de pénétration professionnel (configuration
> de production réelle, injection exhaustive outillée — hors de portée
> depuis cet environnement). **Décompte final recompté par script** : **97
> items, 83 faits (86 %)** · **2 partiels** · **6 incertains/non revérifiés**
> · **6 non faits**.

> **Complément du 06/09/2026** : les deux mécanismes de durcissement déjà
> construits mais partiellement déployés (rôle Postgres non superutilisateur
> pour `audit_log`, chaînage de hash tamper-evident des logs) sont étendus à
> tous les services concernés — respectivement **6/6** (campagne, config,
> auth, abonné ajoutés à paiement/facturation) et **9/9 composants**
> (abonne, campagne, facturation, paiement, notification, config, reporting
> ajoutés à Auth+Gateway). Même mécanisme exact partout, aucune variation ;
> vérifié par une suite d'intégration Postgres réelle par service (pas
> seulement une lecture de code). Ne change le statut d'aucun item (les deux
> items concernés étaient déjà `fait` sur leur périmètre initial), mais
> referme les deux limites explicitement notées la veille comme empêchant
> un item de passer de `fait` à un `fait` sans réserve — seule
> l'observabilité (§I, explicitement différée) reste un obstacle à un audit
...
- [x] Externaliser **tous** les secrets vers un coffre et retirer les valeurs en dur de `docker-compose.yml`. *(M)* — **✅ Refait — PR #198 (05/09/2026).** Les 5 valeurs de repli insécures identifiées par `docs/CONFORMITE_SOC2_OWASP.md` (`PII_ENCRYPTION_KEY`, `DJANGO_SECRET_KEY`, `INTERNAL_GRPC_KEY`, `POSTGRES_PASSWORD`, `WHATSAPP_INTERNAL_API_KEY`) sont retirées de `docker-compose.yml`, remplacées par `${VAR:?message}` — le fail-fast Python de l'item voisin se déclenche désormais réellement, plus aucun secret exploitable en clair dans le dépôt. La clé Fernet PII déjà compromise (committée en clair avant ce correctif) a été rotée en direct sur les données réelles via une migration dédiée (`services/abonne/abonnes/migrations/0008_rotate_pii_encryption_key.py`), zéro perte de données. Historique : PR #169 avait remplacé les littéraux en dur par des valeurs de repli, ce qui neutralisait le fail-fast en pratique — écart trouvé par `docs/CONFORMITE_SOC2_OWASP.md`, repassé en partiel le 04/09, refermé le 05/09.
...
- [x] Activer **mTLS** entre tous les services gRPC ou déployer un service mesh. *(L)* — **✅ Fait — PR #168 (ouverte, non fusionnée, 03/09/2026).** CA interne + certificat généré (`scripts/generate-grpc-certs.sh`), `add_secure_port`/`secure_channel` sur les 9 composants avec repli en clair explicite pour les tests en mémoire. 1131/1131 tests verts, aucune régression. Vérification live (`docker compose up` réel) encore en attente, une stack tournait déjà localement au moment du développement. **Complété le 05/09/2026 (PR #199)** : le repli en clair était jusqu'ici silencieux si les certificats manquaient — désormais un avertissement est journalisé à chaque occurrence, et `GRPC_TLS_REQUIRED=true` permet un refus explicite au démarrage en production plutôt que de retomber sur du gRPC en clair sans le signaler.
...
- [x] **Rate limiting** sur login / refresh / OTP / reset. *(M)* — **✅ Fait — PR #106 (2026-07-21).** `nginx/default.conf:10` `limit_req_zone ... rate=30r/s` + `limit_req_status 429`. Par IP au niveau nginx. **Complété le 05/09/2026 (PR #200)** : le throttle par identifiant métier qui manquait sur la *demande* d'OTP/réinitialisation (recherche `docs/CONFORMITE_SOC2_OWASP.md` §3.1 A04/API4/API6) est comblé par `services/auth/comptes/throttle.py` (verrou Redis `SET NX EX`, 1 requête/60 s), câblé sur `request_otp_by_phone` et `request_password_reset` — en plus du `limit_req` nginx par IP déjà en place.
...
- [x] **Journal d'audit métier immuable**. *(M)* — **✅ Fait, 05/09/2026 — les 6 services prévus par la conception §10.7 sont couverts.** Propagation d'identité (étape 1, `IdentityInterceptor`/`get_caller()`) déjà sur les 9 composants depuis le 04/09. Étape 2 (`AuditLog` + écriture dans la même transaction que le changement métier) livrée pour paiement/facturation (04/09, PR #193) puis étendue le 05/09 aux 4 services restants prévus par la conception : config (PR #205, `UpdateConfig`/`UpdateInfosSociete`), campagne (PR #208, création/clôture de campagne, saisie/correction d'index, affectation d'agent/zones), auth (PR #210, création/modification/désactivation/réactivation d'utilisateur, renvoi d'identifiants — login/OTP/self-service volontairement exclus), abonné (PR #212, création/modification/suspension/réactivation/résiliation d'abonné, remplacement de compteur — anonymisation RGPD volontairement non auditée, même principe que "jamais les factures/paiements" côté conservation légale). Reporting (agrégateur strictement read-only) et Notification (hors périmètre de la conception §10.7) n'ont délibérément pas reçu d'`AuditLog`. Étape 3 (immuabilité base) : **corrigée en profondeur pour paiement/facturation** (voir item dédié ci-dessous, PR #215 — la limite décrite plus bas dans cette même ligne jusqu'au 04/09 était en réalité pire que documenté : le rôle Postgres n'était pas seulement propriétaire mais **superutilisateur**, ce qui contourne toute ACL indépendamment de la propriété) ; **campagne/config/auth/abonné utilisent encore l'ancien REVOKE symbolique** (non corrigé, le mécanisme générique existe désormais dans `libs/sgfe_common/sgfe_common/db_hardening.py` pour qu'un futur correctif l'applique aux 4 services restants sans le redécouvrir).

- [x] **Immuabilité réelle du journal d'audit** (rôle Postgres non superutilisateur). *(M)* — **✅ Fait, 05/09/2026 (PR #215) — pour paiement et facturation.** Item ajouté à cette checklist : l'investigation a montré que la limite documentée jusqu'ici (« le rôle applicatif est propriétaire de la table ») était **plus grave qu'elle ne le disait** — le rôle `POSTGRES_USER` de l'image Postgres officielle est un **superutilisateur** (attribut hérité du bootstrap `initdb`), qui contourne TOUT contrôle d'accès, pas seulement la propriété d'une table (vérifié empiriquement sur un conteneur Postgres jetable : `ALTER TABLE ... OWNER TO <tiers>` + `REVOKE ALL` ne bloque pas davantage un superutilisateur qu'un simple `REVOKE`). Mécanisme retenu, **sans toucher à `docker-compose.yml` ni introduire de nouveau secret** : chaque service crée (via migration `RunPython`, sous le rôle propriétaire) un second rôle Postgres `NOLOGIN` non superutilisateur (`<rôle>_runtime`), `SELECT, INSERT` uniquement sur `audit_log`, tous les droits sur le reste du schéma (existant + futur, via `ALTER DEFAULT PRIVILEGES`) ; la connexion applicative (`grpc_server`, jamais `migrate`) y bascule via `SET ROLE` dès l'établissement de chaque connexion (`<app>/db_hardening.py`, signal Django `connection_created`) — `SET ROLE` change l'identité effective de la session sans changer sa connexion réseau, donc `enregistrer_audit` reste dans la même transaction que la mutation métier qu'il documente. **Vérifié réellement** : suite d'intégration contre un Postgres jetable (`test_db_hardening_postgres.py`, gatée `FORCE_POSTGRES_TESTS`, tourne dans `test-paiement`/`test-facturation`) prouvant qu'après `SET ROLE`, `UPDATE`/`DELETE` échouent (`permission denied`) et que la session perd réellement `rolsuper`. Mécanisme générique dès le départ (`libs/sgfe_common/sgfe_common/db_hardening.py`, synchronisé par `scripts/sync-db-hardening-lib.sh`, vérifié en CI comme `sgfe_common.grpc_auth`) — étendu le 06/09/2026 à **campagne, config, auth et abonné** (mêmes vérifications réelles : suite d'intégration Postgres par service, `permission denied` confirmé après `SET ROLE`) : **6 des 6 services concernés par `AuditLog` isolés par ce mécanisme**, plus aucun `REVOKE` symbolique sans effet.
- [x] **Journalisation de sécurité** centralisée et inviolable. *(M)* — **✅ Fait, 05/09/2026.** Les deux branches de la conception §10.7 sont désormais livrées. **Centralisation** : nouveau RPC `EnregistrerEvenementSecurite` (`proto/auth_service.proto`, stubs régénérés pour `services/auth/proto/` et `gateway/proto/`) — la gateway pousse désormais chaque refus de `require_role` (action `ROLE_REFUSE`) et chaque échec de validation de jeton (`require_auth`/`ValidateToken`, action `TOKEN_INVALIDE`) vers l'`AuditLog` du service Auth (propriétaire naturel de l'identité — conception retenue : TOUS les événements de sécurité gateway y sont centralisés, sans résolution dynamique de "service concerné" par domaine métier), écriture append-only dans sa propre transaction dédiée (`comptes/audit.py::enregistrer_evenement_securite`, distincte de `enregistrer_audit` — aucune mutation métier associée). Appel **best-effort et non bloquant**, même patron que `publish_user_event`/`event_publisher.py` : un échec (Auth indisponible) est capturé et journalisé en avertissement, ne fait jamais échouer la requête GraphQL en cours — testé (mock qui lève une exception, requête aboutit quand même). Le logger local `security` reste alimenté dans TOUS les cas, inchangé, comme filet — jamais remplacé. **Inviolabilité** : nouveau mécanisme générique de chaînage de hash SHA-256 (`libs/sgfe_common/sgfe_common/log_integrity.py`, même patron que `grpc_auth.py`/`db_hardening.py` — source canonique + `scripts/sync-log-integrity-lib.sh` + job CI `check-log-integrity-lib-drift`) : un `logging.Formatter` calcule `sha256(hash_précédent + ligne)` et l'ajoute en suffixe (`log_hash=...`) de chaque ligne écrite dans le fichier de log, câblé initialement sur **Auth et Gateway**, puis étendu le 06/09/2026 aux 7 composants restants (abonne, campagne, facturation, paiement, notification, config, reporting) — **9 des 9 composants Django/Gateway du projet couverts**, même mécanisme exact, aucune variation. Outil de vérification dédié (`sgfe_common/verifier_chaine_logs.py`), testé : logs valides → vérifié OK, une ligne modifiée ou supprimée au milieu → détecté comme rompu. **Limites honnêtes, documentées en tête de `log_integrity.py`** : ce n'est PAS un WORM (un accès root peut réécrire fichier + hash de façon cohérente), PAS une signature externe, et l'état de chaînage vit en mémoire process — pas de persistance entre deux redémarrages (v1 assumée). Nouveau document `docs/PREUVES_SOC2.md` déclare formellement, service par service, depuis quand chaque preuve est collectée.
...
- [ ] 🟡 **Démarrer la collecte de preuves SOC 2**. *(continu)* — **Élargie le 05/09/2026, et formellement déclarée dans un nouveau document.** Les 6 services prévus par la conception §10.7 sont désormais audités (voir item J ci-dessus). **Mise à jour du 06/09/2026** : les deux limites explicitement notées ici la veille sont désormais closes — l'immuabilité DB réelle (rôle Postgres non superutilisateur) couvre maintenant les 6/6 services concernés (campagne/config/auth/abonné ajoutés, plus de `REVOKE` symbolique sans effet), et le chaînage de hash tamper-evident couvre les 9/9 composants Django/Gateway (plus seulement Auth+Gateway). `docs/PREUVES_SOC2.md` déclare formellement, service par service, depuis quelle date exacte chaque preuve est collectée en continu — base vérifiable pour un futur auditeur SOC 2 Type II qui doit chiffrer une période d'observation, plutôt qu'une affirmation narrative. Reste hors périmètre par construction (Reporting/Notification, jamais destinataires d'`AuditLog`) ou toujours à zéro (§I observabilité) — **c'est désormais le seul point qui ferait échouer un Type II aujourd'hui** (CC7), tous les autres contrôles listés ici étant maintenant complets sur leur périmètre prévu. Reste `partiel` par nature : *(continu)*, une période d'observation ne se clôt pas sur commande — voir §I, explicitement différé.

> 📄 **Nuancé le 04/09/2026** par `docs/CONFORMITE_SOC2_OWASP.md` (§3.4 CC7, avertissement de portée en tête de document — diagnostic de préparation, **pas une certification**) : confirme le travail ci-dessus (PR #193) mais rappelle qu'il ne couvre que 2 services sur 8, que l'immuabilité base n'est pas effective dans la configuration actuelle (limite déjà documentée ci-dessus), et qu'il exclut explicitement l'observabilité — **CC7 reste le critère qui ferait échouer un audit Type II aujourd'hui**. Le même rapport relevait aussi (§3.1 A01, §3.3 V8) qu'aucun service gRPC interne ne revalidait le rôle utilisateur : la gateway restait l'unique point de décision RBAC (aveu explicite en commentaire, `services/auth/comptes/services.py:193`). **Corrigé le 05/09/2026 (PR #199)** : revalidation du rôle appelant (log d'avertissement, non bloquante) ajoutée sur les 5 RPC les plus sensibles (`EnregistrerPaiement`, `AnnulerPaiement`, `EnregistrerPaiementAbonne`, `DeactivateUser`, `UpdateTarif`), en s'appuyant sur l'identité propagée par PR #193 — la gateway reste le décideur RBAC principal, ceci n'est qu'un filet de sécurité qui journalise plutôt que de bloquer, tant que la propagation d'identité ne couvre pas tous les appels légitimes.
...
- [x] 🔗 Vrais **tests e2e Playwright**. *(M)* — **✅ Fait — confirmé en direct le 05/09/2026, après 4 itérations de vrais échecs CI diagnostiqués un par un (jamais forcés au vert).** PR #143 (31/08) ajoute un spec saisie-index. Backend PR #189 (04/09) ajoute `WHATSAPP_DISABLE_SEND_FOR_TESTS` ; le frontend lève le `test.skip()` du spec paiement et ajoute 3 specs (`espace-abonne`, `dashboard-consultation`, `abonnes-gestion`), les 4 gatées `E2E_LIVE_BACKEND`. PR #217 (backend, même jour) corrige 3 manques de seed (aucune campagne `EN_COURS`, aucun abonné, date d'impayé buguée). Le job `e2e` (`SGFE-frontend#175`) a ensuite été étendu pour exécuter réellement les 4 specs contre la stack live — 4 tentatives CI, 4 causes réelles distinctes trouvées et corrigées, aucune contournée : **(1) un vrai bug de production** — `impayes-list.component.ts` appelait `abonnes` (réservé ADMIN) dans un `Promise.all`, cassant silencieusement l'écran `/impayes` pour tout utilisateur COMPTABLE (rôle pourtant autorisé sur cette route) depuis l'introduction de l'écran ; corrigé en réutilisant `abonneNom`/`abonneNumero`, déjà résolus côté Gateway pour ce rôle exact (`_enrichir_factures`) ; **(2)** une valeur de recherche e2e incorrecte (`AB-9001` au lieu du `AB-9003` réservé) ; **(3)** deux specs ADMIN/COMPTABLE ciblaient les sélecteurs de la vue tableau d'`app-data-table`, masquée sur les viewports mobiles (< 1024px) au profit d'une vue carte — restreints à `chromium`, seul projet desktop ; **(4)** `terrain-saisie-index.spec.ts` consomme un vrai relevé par exécution réussie, et le seed n'en fournit que 2 : exécuté sur 3 projets Playwright (workers en série), le 3e trouvait systématiquement une tournée vide — pas un flake, un budget de seed dimensionné pour 2 exécutions ; restreint aux 2 projets mobiles (AGENT/terrain est de toute façon mobile-only, CLAUDE.md). Un vrai bogue de synchronisation de test (assertion lue avant la fin du debounce de recherche) corrigé au passage. CI finale : 17 tests, tous verts ou skippés à bon escient (aucun test.skip masquant un vrai échec).
...
- [ ] 🟡 **Test de pénétration** avant go‑live. *(M)* — **Vérification dynamique ciblée faite les 05/09/2026 (deux passages), pas un pentest professionnel.** Nouveau `docs/PENTEST_LITE.md`. **Passage 1** : contre la stack `docker compose` reconstruite (le premier passage avait tourné par erreur contre une image obsolète, antérieure à PR #200 — incident méthodologique découvert et corrigé avant de continuer), confirmé en conditions live : throttle OTP/reset-password (PR #200), RBAC gateway (AGENT rejeté sur `updateTarif`/`deactivateUser`), validation de signature JWT RS256 (résiste à la falsification et à l'élévation de rôle par modification du payload), limite d'alias GraphQL (50), en-têtes de sécurité HTTP, rate limiting nginx (30r/s burst=60, 10/80 requêtes rejetées en rafale), isolation réseau gRPC/Postgres, fail-closed WhatsApp. **Passage 2, complémentaire** : second incident méthodologique découvert (la stack tournait sans aucun certificat mTLS — `./certs/` jamais généré sur cette machine, repli en clair documenté mais actif) ; certificats générés et services redémarrés, puis testé en conditions live depuis l'intérieur du réseau Docker : mTLS mutuel avec validation de CA (canal en clair refusé, certificat non signé par la CA refusé, certificat légitime accepté) ; IDOR sur les 3 routes tokenisées de l'espace abonné (vue JSON, PDF de facture, confirmation de paiement) avec deux comptes de démo réels — toutes bloquées correctement (404 sur accès croisé) ; sondage d'injection ciblé (11 charges SQLi/NoSQL/shell/path-traversal sur 2 points d'entrée — login GraphQL et paramètre token de l'espace abonné) sans succès, path traversal bloqué par nginx. Effet de bord mineur assumé : un paiement mock de 1 unité confirmé sur une facture de démo (flux sandbox). Une observation d'architecture (pas une vulnérabilité) : `whatsapp-service:3000` publié directement sur l'hôte en plus de nginx, protégé par clé API. **Toujours non couvert, à faire par un prestataire spécialisé** : injection exhaustive (fuzzing outillé, pas un sondage de 11 charges), SSRF, désérialisation, configuration de production réelle (vraie PKI, `DEBUG=False`, déploiement EC2 réel).
...
- [x] **RGPD** : export + effacement/anonymisation. *(M)* — **✅ Fait — PR #179 (abonné, 03/09) puis étendu PR #213 (utilisateurs internes, 05/09).** Abonné : `ExporterDonneesAbonne` (droit à la portabilité, dégradation gracieuse par section) et `AnonymiserAbonne` (droit à l'effacement, uniquement `RESILIE`, jamais factures/paiements). Auth (05/09) : même esprit pour les comptes internes (ADMIN/AGENT/COMPTABLE/SUPERVISEUR) — `ExporterDonneesUtilisateur` et `AnonymiserUtilisateur` (refuse si le compte est actif ; anonymise `username`/`email`/`phone_number`, dérivés de l'UUID car uniques en base, contrairement à l'Abonné où une valeur littérale suffisait). Mutations gateway ADMIN uniquement des deux côtés. **Non couvert** : complétude juridique des champs exportés, non validée par un juriste — mécanisme technique seulement.
- [x] Politique de **rétention des données** (utilisateurs internes). *(S)* — **✅ Fait — PR #213 (05/09/2026), durée validée explicitement par l'utilisateur du projet : 3 ans après désactivation.** Nouveau champ `User.date_desactivation` (posé/effacé symétriquement par `deactivate_user`/`reactivate_user`), job planifié quotidien (`purge_rgpd_job`, 04h00, verrou `pg_advisory_lock` — même patron que les autres crons du dépôt) anonymisant automatiquement tout compte désactivé depuis plus de 3 ans, dégradation par utilisateur (un échec n'empêche pas les autres). **Reste non couvert** : la même politique pour les données abonné (portée initiale de cet item), toujours non tranchée.
...
- [x] i18n : externaliser les messages backend codés en dur. *(M)* — **✅ Fait — mécanisme d'externalisation, pas de traduction.** 113 littéraux destinés à l'utilisateur final (erreurs de validation métier, messages d'exception remontés jusqu'à une réponse gRPC) enveloppés dans `django.utils.translation.gettext_lazy` (`as _`) sur les 8 services — auth 27, campagne 27, facturation 24, paiement 20, abonne 8, notification 6, config 1, reporting 0 (aucun littéral utilisateur dans ce service, purement read-only). Fichiers concernés selon le service : `services.py`, `grpc_server.py`, `grpc_interceptors.py` (mapping exception → code gRPC, message parfois codé en dur à cet endroit plutôt que dans `grpc_server.py`), `repositories.py` (messages `ObjectDoesNotExist`), `validators.py`, `throttle.py`, `models.py`. `serializers.py` : 0 sur les 8 (pures fonctions de mapping modèle → protobuf, aucune validation utilisateur). `LANGUAGE_CODE = "fr-fr"` (7 services l'avaient encore à `"en-us"` alors qu'aucune traduction n'existe), `USE_I18N = True` et `LOCALE_PATHS` posés dans chaque `settings.py`, dossier `locale/` créé (vide) par service. **Comportement inchangé, vérifié réellement** : 1386/1386 tests verts sur les 8 services sans modification d'aucun test, y compris ceux qui comparent un message d'exception exact — piège trouvé et corrigé en cours de route (`total, _, _ = fn()` masquait l'alias `_` de gettext dans la même fonction, remplacé par des noms explicites). **Hors périmètre, non fait délibérément** : la traduction elle-même (aucun fichier `.po`/`.mo`, aucun texte anglais fourni) — commande documentée dans `CLAUDE.md` racine pour plus tard (`manage.py makemessages -l en` / `compilemessages`).
...
|---|---|---|---|
| 🚦 **Go production (technique)** | Aucun secret exposé, périmètre gRPC verrouillé, système déployable + sauvegardé, bugs bloquants corrigés | **Tous les P0** + F, I (min. `/metrics` + healthchecks + logs) + K (e2e smoke vert) | 🟡 **Bloqué sur un point, plus deux.** P0 à 26/27 (**§8·A refermé le 05/09/2026 — PR #198**, valeurs de repli insécures retirées de `docker-compose.yml`, fail-fast réel ; seule la rotation de la clé Brevo reste ❓, hors code, non vérifiable depuis le dépôt) — mTLS vérifié en direct (+ fail-fast `GRPC_TLS_REQUIRED` et avertissement journalisé sur repli en clair, PR #199), healthchecks complets sur les 9 composants. **I (observabilité, hors healthchecks) reste à zéro, jamais entamé** — confirmé indépendamment par `docs/CONFORMITE_SOC2_OWASP.md`/`docs/CONFORMITE_CICD.md` le 04/09 — bloque toujours cette porte à elle seule, `/metrics`/`TracerProvider`/logs structurés absents. |
...
| 🛡️ **Prêt pour l'audit SOC 2 Type II** | Contrôles en place **et** preuves accumulées sur la période d'observation | P0 (sécurité) + J (audit trail) + I (monitoring) + K (pentest) + rate limiting (C) | ⛔ **Toujours loin, mais J est désormais fait.** mTLS + PII + TLS + RGPD (abonné + interne) traités. **J** (05/09) : identité propagée sur les 9 composants + `AuditLog` immuable sur les 6 services prévus (paiement/facturation/campagne/config/auth/abonné) + immuabilité base réellement corrigée pour paiement/facturation (rôle Postgres non superutilisateur, PR #215 — campagne/config/auth/abonné gardent l'ancien REVOKE symbolique) + rétention/horodatage fiable sur les 9 composants + logger sécurité gateway partiel — ✅ fait, plus 🟡. **I (observabilité) reste à zéro, jamais entamé**, K‑pentest non fait — ce sont les seuls vrais blocages restants. **Diagnostic indépendant complet le 04/09** : `docs/CONFORMITE_SOC2_OWASP.md` (CC1-CC9 + OWASP Top 10/API/ASVS) confirme ce même verdict sans surprise — CC7/CC4 (surveillance) est le seul critère qui ferait échouer un Type II sans ambiguïté ; explicitement un diagnostic de préparation, pas une certification. |
...
>
> **Mise à jour du 5 septembre 2026** : plan de remédiation des deux rapports exécuté (6e revue de fraîcheur en tête de document) — **externalisation des secrets** (§8·A, PR #198) et **Trivy** (§8·K, PR #197 + #164) repassent tous deux de partiel à fait, portant le total à **76 (79 %)**. Les autres correctifs du même lot (défense en profondeur gRPC, mTLS fail-fast, chiffrement PII notification, throttle OTP/reset, validation GraphQL, provenance SLSA) n'ont pas d'item dédié dans cette checklist et ne changent donc pas le décompte — voir la 6e revue de fraîcheur pour le détail.
>
> **Mise à jour du 5 septembre 2026 (i18n)** : **§Q · i18n backend** repasse de non-fait à fait — 113 littéraux utilisateur (erreurs de validation métier, messages d'exception remontés jusqu'à une réponse gRPC) enveloppés dans `gettext_lazy` sur les 8 services (auth 27, campagne 27, facturation 24, paiement 20, abonne 8, notification 6, config 1, reporting 0), mécanisme Django standard câblé (`LANGUAGE_CODE`, `USE_I18N`, `LOCALE_PATHS`, dossier `locale/` par service) — **externalisation seulement, aucune traduction fournie** (pas de fichier `.po`/`.mo`). 1386/1386 tests verts sur les 8 services sans modification d'aucun test. Portant le total à **81 (84 %)**.
>
...
>
> **Ce qui reste, 4 septembre** : piste d'audit (§J) entamée mais **incomplète** — `AuditLog` manque encore sur campagne/abonné/auth/config (une PR par service, dans cet ordre, prévu par la conception §10.7), la journalisation de sécurité ne couvre que la gateway, et le REVOKE niveau base a une limite documentée (rôle applicatif = propriétaire de la table dans la configuration actuelle). **Mise à jour du 5 septembre : cette dernière limite est corrigée pour paiement et facturation** (rôle `_runtime` non superutilisateur + `SET ROLE`, voir §J ci-dessus) — `AuditLog` sur campagne/abonné/auth/config reste, seul point non fait de cet item. **Observabilité (§I)** reste le seul chantier à zéro, jamais entamé. Tarification par tranches, pénalités de retard et estimation automatique des compteurs — **déclinés explicitement par l'utilisateur** le 3 septembre (staging iso-prod, CGU et multi-tenant classés « pas d'actualité », pas refusés). **Ce qui reste, P2** : uniquement l'environnement de staging iso-prod (non fait, explicitement pas d'actualité pour l'instant).
>
...
> **Mise à jour du 5 septembre 2026** : **Journalisation de sécurité centralisée et inviolable** (§J) passe de 🟡 partiel à ✅ fait — nouveau RPC `EnregistrerEvenementSecurite` qui centralise les événements de sécurité de la gateway dans l'`AuditLog` du service Auth (best-effort, non bloquant), et nouveau mécanisme de chaînage de hash tamper-evident (`libs/sgfe_common/sgfe_common/log_integrity.py`), câblé sur Auth et Gateway. Un seul item §J change de colonne (P1 : partiel→fait). Nouveau document `docs/PREUVES_SOC2.md`, qui décline formellement l'item "Démarrer la collecte de preuves SOC 2" (reste 🟡, par nature continu). Combiné le même jour à l'externalisation i18n (§Q, voir plus haut) : total **82 (85 %)** faits, 2 partiels, sur les mêmes 97 items — recompté avec `scripts/compter_audit.py`. L'observabilité (§I) reste le seul chantier à zéro, et reste le seul point qui ferait échouer un audit SOC 2 Type II aujourd'hui.
