...
- [ ] 🟡 **Démarrer la collecte de preuves SOC 2**. *(continu)* — **Élargie le 05/09/2026, et formellement déclarée dans un nouveau document.** Les 6 services prévus par la conception §10.7 sont désormais audités (voir item J ci-dessus), la centralisation des événements de sécurité gateway→Auth et le chaînage de hash tamper-evident (Auth + Gateway, voir item J ci-dessus) démarrent leur propre période d'observation ce jour. **Nouveau : `docs/PREUVES_SOC2.md`** déclare formellement, service par service, depuis quelle date exacte chaque preuve est collectée en continu — base vérifiable pour un futur auditeur SOC 2 Type II qui doit chiffrer une période d'observation, plutôt qu'une affirmation narrative. Reste hors périmètre par construction (Reporting/Notification, jamais destinataires d'`AuditLog`), non effectif malgré les apparences (immuabilité DB réelle limitée à paiement/facturation — campagne/config/auth/abonné gardent un `REVOKE` symbolique sans effet contre un rôle Postgres superutilisateur), ou toujours à zéro (§I observabilité, chaînage de hash limité à 2 composants sur 9) — **c'est le point observabilité, seul, qui ferait échouer un Type II aujourd'hui** (CC7), quelle que soit la maturité des autres contrôles listés ici.
