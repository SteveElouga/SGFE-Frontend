...
- [ ] 🔗 Vrais **tests e2e Playwright**. *(M)* — **🟡 Partiel, étendu mais toujours incomplet.** PR #143 (fusionnée 31/08) ajoute un spec saisie-index (gaté `E2E_LIVE_BACKEND=1`, skip propre en CI). Le spec paiement était écrit mais en `test.skip()` **permanent** : vérifié dans le code backend que l'enregistrement d'un paiement déclenche un envoi WhatsApp réel et inconditionnel (`_propager_versement`), sans possibilité de l'éviter côté UI — jamais exécuté contre un backend réel, à raison. **Corrigé le 04/09/2026** : backend PR #189 ajoute `WHATSAPP_DISABLE_SEND_FOR_TESTS`, qui fait simuler `send()`/`send_with_pdf()` (`notifications/whatsapp_client.py`) sans jamais contacter le vrai `whatsapp-service` ; le même jour, le frontend (commit `a9d2b7b`) lève le `test.skip()` inconditionnel de `paiement-encaissement.spec.ts` et le fait passer au même gate `E2E_LIVE_BACKEND` que saisie-index, plus 3 nouveaux specs (`espace-abonne`, `dashboard-consultation`, `abonnes-gestion`). **Reste non fait pour autant** — pas seulement un skip permanent : `.github/workflows/ci.yml` (frontend, job `e2e`) lance `npx playwright test` sans jamais poser `E2E_LIVE_BACKEND` ni démarrer de stack `SGFE-backend`, donc ces 4 specs « live » restent skip **à chaque run CI**, jamais exécutés automatiquement — seuls `connexion.spec.ts` et `espace-abonne.spec.ts` (sans dépendance backend réelle) tournent réellement en CI aujourd'hui.
